...
Table \ref{tab:calibration} separates the two observed targets, the parameters
chosen to match them, and the remaining benchmark inputs. Formal production
uses a capital share of 0.33 and annual depreciation of 0.06. A four-percent
real bond rate implies $\beta=1/1.04$. The CES elasticity is 1.5: formal and
informal goods are substitutes, but far from perfect substitutes. The
consumption tax reaches 15 percent of informal expenditure.
...
\midrule
\multicolumn{4}{l}{\textit{Observed calibration targets}} \\
Public pension spending & $G^S/Y$ & 0.057 & Share of GDP \\
Labor informality & $1-n_F$ & 0.554 & Share of employment \\
\addlinespace
\multicolumn{4}{l}{\textit{Parameters calibrated to the observed targets}} \\
Outlay per older adult & $\bar g_o$ & 0.426 & Model consumption units \\
Formal-work fixed cost & $\kappa$ & 0.633 & Model utility units \\
\addlinespace
\multicolumn{4}{l}{\textit{Benchmark inputs and provisional assumptions}} \\
...
\item Notes: The model jointly chooses $\bar g_o$ and $\kappa$ to match the two
observed targets. The value $\bar g_o=0.426$ is expressed in model consumption
units per older adult; it is not a share of GDP or a tax rate. This calibration
is performed in an environment that assigns 45 percent of required revenue to
consumption taxes, 35 percent to payroll taxes, and 20 percent to capital-income
taxes. That mix is neither an observed target nor an estimate or policy result.
Effective tax rates and quantities in model units should not be read as
statutory Colombian rates or quantities in pesos.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\input{results/optimal_mix_table.tex}
